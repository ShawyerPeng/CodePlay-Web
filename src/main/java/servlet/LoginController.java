...
public class LoginController {
    ...
    /**
     * 验证验证码
     * 通过@RequestParam获取请求的参数,value对应的key,required表明字段必填
     * @return ModelAndView
     */
    @RequestMapping(value = "/verifyCode")
    public ModelAndView verifyCode(@RequestParam(value = "code") String code, HttpServletRequest request){
        ModelAndView mav = new ModelAndView("/ImageCode");
        System.out.println("验证码：" + code);
        // 获取session中保存的验证码
        String oldCode = (String) request.getSession().getAttribute("validateCode");
        if(code.equals(oldCode)){ // 验证通过
            mav.addObject("message", "验证成功");
        }else{
            mav.addObject("message", "验证失败");
        }
        return mav;
    }
}